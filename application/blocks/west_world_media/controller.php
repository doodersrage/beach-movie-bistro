...
class Controller extends BlockController{
	...
	private function getWWMListingData(){
		...
		// gather movie data
		$movie_data = $this->get_data('http://webservice.cinema-source.com/3.8/?apikey=THBMB&query=theater&schedule=yes&house_id=34510&sd=yes&showdate='.$startDate.'&enddate='.$endDate);
				...
				if(!empty($movie['movie_id'])){
					...
					foreach($rtsListing['ShowSchedule']['Films']['Film'] as $film){
						...
						if($film['CSCode'] == $movie['movie_id']){
							...
							// add date/time values to selectable date hash to save front-end processing of options
								...
								foreach($tickets as $TI){
									...
									if(!empty($ticket) && empty($ticket['HideOnInternet'])){
										...
										if($movie['showtimes']['@attributes']){
											...
											$stDate = $stDate[2] . sprintf("%02d", $stDate[0]) . sprintf("%02d", $stDate[1]);
											if($stDate == $showDate){
